...
| TUSHARE_ADJUSTED_PRECLOSE | Tushare除权昨收语义 | 数据质量边界 | FROZEN | selected_provider, pre_close, previous raw close | Tushare selected_provider的除权`pre_close`不强制等于上一交易日raw close；其他Provider保留连续性校验 | 否 | 否 | 是 | test_warehouse_validate | [[03_Decisions/ADR-006-multi-source-warehouse-reconciliation]] |
