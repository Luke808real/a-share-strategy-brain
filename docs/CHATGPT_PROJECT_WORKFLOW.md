...
1. 创建或使用“A股首板回踩再启动策略”Project，并把当前长期聊天移动到该Project。
   新 Agent 先读取 [[00_Project/AGENT_HANDOFF]]、[[00_Project/CURRENT_STATE]] 与
   [[00_Project/PROJECT_CHARTER]]，不要求扫描整个 Vault。
2. 首次上传 `exports/LLM_CONTEXT_PACK.md`；该包前部必须来自 Agent Handoff、
   Current State、Project Charter、冻结策略摘要与状态机；后续优先上传
...
5. 对话结论仍需归档、人工审核，不能由ChatGPT直接改变冻结真源。当前状态更新必须
   遵守 [[00_Project/STATE_UPDATE_PROTOCOL]]。
